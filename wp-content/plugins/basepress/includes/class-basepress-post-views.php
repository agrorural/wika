			...
			if ( 'knowledgebase' == $post_type ) {
				...
				<script type="text/javascript">
					jQuery(document).ready(function() {
						jQuery( '<option>' ).val( '' ).text( '<?php _e( '─────────' ); ?>').prop( 'disabled', true ).appendTo( "select[name='action']" );
						...
						jQuery( '<option>' ).val( '' ).text( '<?php _e( '─────────' ); ?>').prop( 'disabled', true ).appendTo( "select[name='action2']" );
				...
				switch ( $action ) {
					...
					case 'reset_views':
						...
						foreach ( $post_ids as $post_id ) {
							...
							if ( ! $this->reset_views( $post_id ) ) {
								wp_die( __( 'Error resetting views.' ) );
							}
					...
					case 'reset_views_all':
						...
						foreach ( $all_posts as $post ) {
							...
							if ( ! $this->reset_views( $post_id ) ) {
								wp_die( __( 'Error resetting votes.' ) );
							}
